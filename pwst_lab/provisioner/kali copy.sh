...
# -----------------------------------------------------------------------------
logger "mttaggart pwst-resources"

sudo -H -u ${LOCAL_USER} /bin/bash -c "cd ${REPO_DIR} && \
  git clone https://github.com/danf42/pwst-resources.git && \
  cd ${REPO_DIR}/pwst-resources/kali-setup/ && \
  bash ./setup.sh"

# WARNING! Password in plain text
logger "Make fish the default shell"
echo ${LOCAL_USER_PASSWD} | sudo -H -u ${LOCAL_USER} chsh -s /usr/bin/fish

# Enhance fish history
cat > "${LOCAL_USER_HOME}/.config/fish/functions/history.fish" << EOF
function history
    builtin history --show-time='%F %T '
end
EOF

chown ${LOCAL_USER}:${LOCAL_USER} ${LOCAL_USER_HOME}/.config/fish/functions/history.fish
...
wget https://github.com/NationalSecurityAgency/ghidra/releases/download/Ghidra_10.2.2_build/ghidra_10.2.2_PUBLIC_20221115.zip -O /tmp/ghidra.zip
...
sudo -H -u ${LOCAL_USER} /bin/bash -c "cd ${REPO_DIR} && \
  git clone https://github.com/s0med3v/Arjun.git"
...
wget https://github.com/java-decompiler/jd-gui/releases/download/v1.6.6/jd-gui-1.6.6.deb -O /tmp/jdgui.deb
